...
for brand_id in brands: 
    ...
    dataloader_test_dict[brand_id] = DataLoader(dataset_test_dict[brand_id], batch_size=2048, shuffle=False, collate_fn=my_collate_fn)
print('test data finish')

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = MyModel(dnn_config).to(device) 
            ...
            if (total_step+1)%7000 == 0:
                with torch.no_grad():
                    ...
                    for brand_id in brands:
                        ...
                        test_preds = []
                        test_targets = []
                        for data, target in test_loader_dict[brand_id]:
                            output  = model(data)
                        ...
                        # 如果需要保存结果到文件
                        with open(f'{log_dir}/models_{brand_id}_top_results_focal_{total_step}.txt', 'w') as f:
                            for k, ratio in ratios.items():
                                f.write(f"Top {k} ratio of positive labels: {ratio:.4f}\n")

                torch.save(model.state_dict(), f'models/focal4/model_epoch_{epoch}_{total_step}.pth')
                model.train()
        torch.save(model.state_dict(), f'models/focal4/model_epoch_{epoch}.pth')
    with torch.no_grad():
        model.eval()
        for brand_id in brands:
            #brand_id='b56508'
